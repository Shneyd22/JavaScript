...
document.addEventListener('DOMContentLoaded', async (e) => {
    ...
    data.forEach(element => {
        items_list.insertAdjacentHTML('beforeend', `
            <figure id="${element.id}"class="item">
                <div class="item_img">
                    <img src="${element.img}" alt="img" class="item_img">
                    <div class="item_hover_img">
                        <a href="#" class="item_hover_link">
                ...
                <div class="item_info">
                    <figcaption class="item_title">${element.title}</figcaption>
                    <p class="item_text">${element.text}</p>
                    <p class="item_price">$${element.price}</p>
                </div>
            </figure>
        `)
    });
    ...
    items_list.addEventListener('click', (e) => {
        if (e.target.closest('.item_hover_link')) {
            cart_items.style.display = 'flex';
            const cart_item = data.find(elem => elem.id === e.target.closest('.item').getAttribute('id'));
            cart_items_list.insertAdjacentHTML('beforeend', `
                <div class="cart_item">
                    <img src="${cart_item.img}" alt="img">
                    <div class="cart_item_description">
                        <h2 class="cart_item_description_title">${cart_item.title}</h2>
                        <div class="cart_item_description_list">
                            <p class="cart_item_description_text">Price: <span class="red">$${cart_item.price}</span ></p >
                            <p class="cart_item_description_text">Color: <span class="grey">${cart_item.color}</span></p>
                            <p class="cart_item_description_text">Size: <span class="grey">${cart_item.size}</span></p>
                            <p class="cart_item_description_text">Quantity: <input class="cart_item_description_input" type="number" value="${cart_item.count}"></p>
                        </div >
                        </div >
    ...
    cart_items_list.addEventListener('click', (e) => {
        if (e.target.closest('.cart_item_delete')) {
            const del_item = e.target.closest('.cart_item');
            if (del_item) {
                del_item.remove();
            }
        }
